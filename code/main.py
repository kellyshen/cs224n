...
tf.app.flags.DEFINE_string("mode", "train", "Available modes: train / show_examples / official_eval / recalculate_score")
...
tf.app.flags.DEFINE_integer("self_attn_dim", 1, "Matrices dimensions used in Self-Matching Attention")
...
tf.app.flags.DEFINE_string("attention", "arch2", "basicattn, bidaf, selfattn, rnet, bidaf_selfattn_parallel, bidaf_selfattn_piped, bidaf_rnet_parallel, bidaf_rnet_piped, arch1, arch2, bidaf2")
...
def main(unused_argv):
    ...
    elif FLAGS.mode == "recalculate_score":
        with tf.Session(config=config) as sess:
            # Load best model
