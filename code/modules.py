...
class BiDirectionalAttn2(object):
    """
    Module for BiDirectional Attention. (Refer to BiDAF in 5.1.1)
    ...
    module with other inputs.
    """
    def __init__(self, keep_prob, vec_size, num_keys, num_values):
        """
        Inputs:
          keep_prob: tensor containing a single scalar that is the keep probability (for dropout)
          key_vec_size: size of the key vectors. int
    ...
    def build_graph(self, keys, keys_mask, values, values_mask):
        """
        Keys attend to values.
        For each key, return an attention distribution and an attention output vector.
        Inputs:
        ...
        with vs.variable_scope("BiDirectionalAttn2"):
            ...
            W_q = tf.get_variable("W_q", shape=[self.vec_size, 1],
                initializer=tf.contrib.layers.xavier_initializer()) # Test: different seeds
