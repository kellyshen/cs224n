...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        if self.FLAGS.attention == "BiDAF":
            ...
            # representation against itself
            self_attn_layer = SelfAttn(self.keep_prob, self.FLAGS.hidden_size*4, self.FLAGS.context_len, self.FLAGS.self_attn_dim)
            self_attn_dist, self_attn_output = self_attn_layer.build_graph(basic_blended_reps, self.context_mask) # (batch_size, context_len, hidden_size*4)
            # self_blended_reps is blended_reps_ concatted to self_attn_output
            self_blended_reps = tf.concat([basic_blended_reps, self_attn_output], axis=2) # (batch_size, context_len, hidden_size*8)
            
            # Encode current passage and question information
            encoder = BiRNN(self.FLAGS.hidden_size, self.keep_prob)            
            blended_reps = encoder.build_graph(self_blended_reps, self.context_mask)  # (batch_size, context_len, hidden_size*2)
            ...
            _, self_attn_output = self_attn_layer.build_graph(blended_reps, self.context_mask) # (batch_size, context_len, hidden_size*8)
            # self_blended_reps is blended_reps_ concatted to self_attn_output
            self_blended_reps = tf.concat([blended_reps, self_attn_output], axis=2) # (batch_size, context_len, hidden_size*16)
            ...
            # representation against itself
            self_attn_layer = SelfAttn(self.keep_prob, self.FLAGS.hidden_size*8, self.FLAGS.context_len, self.FLAGS.self_attn_dim)
            self_attn_dist, self_attn_output = self_attn_layer.build_graph(basic_blended_reps, self.context_mask) # (batch_size, context_len, hidden_size*8)
            # self_blended_reps is blended_reps_ concatted to self_attn_output
            self_blended_reps = tf.concat([blended_reps, self_attn_output], axis=2) # (batch_size, context_len, hidden_size*16)
            ...
            r_encoder = RNN(self.FLAGS.hidden_size, self.keep_prob) # (batch_size, contex_len, hidden_size)
            c = tf.matmul(self_attn_dist, self_blended_reps_2) # (batch_size, contex_len, contex_len) * (batch_size, context_len, hidden_size*2)
            h_a = r_encoder.build_graph(c, self.context_mask) # (batch_size, context_len, hidden_size)
    ...
    def get_start_end_pos(self, session, batch):
        ...
        candidates = []
        for i in range(15):
            end_array = np.concatenate((np.zeros((i,self.FLAGS.batch_size)), end_dist.T[:self.FLAGS.context_len - i]), 0).T
            probs = tf.multiply(start_dist, end_array)
